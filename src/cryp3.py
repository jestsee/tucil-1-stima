...
def magic(huruf, angka, visited, words):
    ...
    if len(set) == len(angka):
        ...
        if sum == int(wordi[len(wordi)-1]): 
            ...
            cetakangka(wordi)
            print("\n")
            print(magic.counter,"total tes sampai ditemukan solusi ini")
            found = True
        return
    ...
    for i in range(10): 
        if not visited[i]: 
            visited[i] = True
            angka.append(i)
            magic(huruf, angka, visited, words) 
            angka.pop() 
            visited[i] = False   
...
if len(result) > (maxx(wordcomp)+1): 
    ...
    magic.counter = 0
    magic(set, [], [False for _ in range(10)], wordcomp)
